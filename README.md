...
2. [Cargo Lambda](https://www.cargo-lambda.info/guide/installation.html):
   ...
   # or via cargo (requires Zig for cross-compilation)
...
This cross-compiles to `x86_64-unknown-linux-gnu` and produces a `bootstrap` binary in `target/lambda/lambda/`.
